...
import { 
  ...
  ClipboardList, Wrench, LogOut
} from 'lucide-react';
import { onAuthStateChanged, signOut } from 'firebase/auth';
import { auth } from './firebase';

// Logic & Data
import { useCloudData } from './hooks/useCloudData'; // NEW Hook
import { useLocalStorage } from './hooks/useLocalStorage'; // Keep for device settings
import { 
...
const App = () => {
  ...
  // --- 2. Persistence (Cloud vs Local) ---
  
  // Device-specific settings (Theme & Sidebar stay on the device)
  ...
  // App Data (Synced to Cloud per user)
  const [currency, setCurrency] = useCloudData('currency', 'USD', user);
  const [budgetItems, setBudgetItems] = useCloudData('budget', INITIAL_BUDGET_ITEMS, user);
  const [tasks, setTasks] = useCloudData('timeline', INITIAL_TIMELINE, user);
  const [activity, setActivity] = useCloudData('activity', INITIAL_ACTIVITY, user);
  const [gallery, setGallery] = useCloudData('gallery', INITIAL_GALLERY, user);
  const [rooms, setRooms] = useCloudData('rooms', DEFAULT_ROOMS, user);
  const [categories, setCategories] = useCloudData('categories', DEFAULT_CATEGORIES, user);
  const [renovationPlans, setRenovationPlans] = useCloudData('plans', [], user);
  const [inventory, setInventory] = useCloudData('tools', [], user);
  ...
  const NavItem = ({ id, icon: Icon, label, alert }) => (
    <button onClick={() => setActiveTab(id)} className={`flex items-center gap-3 px-3 py-3 rounded-lg transition-all w-full relative group ${activeTab === id ? 'bg-blue-50 text-blue-600 font-medium dark:bg-blue-900/20 dark:text-blue-400' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-slate-200'} ${isSidebarCollapsed ? 'justify-center' : 'justify-start'}`} title={isSidebarCollapsed ? label : ''}>
      <div className="relative shrink-0"><Icon size={22} />{alert && <span className="absolute -top-1 -right-1 w-2 h-2 bg-red-500 rounded-full border-2 border-white dark:border-slate-900"></span>}</div>
  ...
  // --- Loading & Auth States ---
  if (authLoading) return <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 text-slate-400">Loading re4b...</div>;
  ...
  return (
    <div className="h-[100dvh] overflow-hidden bg-slate-50 dark:bg-slate-950 flex flex-col md:flex-row font-sans text-slate-900 dark:text-slate-100 transition-colors duration-300">
      ...
      <aside className={`hidden md:flex flex-col bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 p-4 shrink-0 fixed h-full z-20 transition-all duration-300 ease-in-out ${isSidebarCollapsed ? 'w-20' : 'w-64'}`}>
        ...
        <div className={`flex items-center gap-2 px-2 mb-8 transition-all duration-300 ${isSidebarCollapsed ? 'justify-center' : ''}`}><div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center text-white font-bold shadow-lg shadow-blue-600/20 shrink-0">r</div>{!isSidebarCollapsed && <span className="font-bold text-xl tracking-tight text-slate-900 dark:text-white whitespace-nowrap overflow-hidden animate-in fade-in slide-in-from-left-2">re4b</span>}</div>
        ...
        <div className="mt-auto space-y-4">
          <div className="px-1 py-4 border-t border-slate-100 dark:border-slate-800">
            <button onClick={() => setIsSettingsOpen(true)} className={`flex items-center gap-3 w-full text-left hover:bg-slate-50 dark:hover:bg-slate-800 p-2 rounded-lg transition-colors group ${isSidebarCollapsed ? 'justify-center' : ''}`} title="User Settings">
              {user.photoURL ? <img src={user.photoURL} alt="User" className="w-8 h-8 rounded-full border border-slate-200 dark:border-slate-700" /> : <div className="w-8 h-8 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-slate-500 dark:text-slate-400 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors shrink-0"><UserCircle size={16} /></div>}
              {!isSidebarCollapsed && <div className="text-sm overflow-hidden"><div className="font-medium dark:text-slate-200 truncate">{user.displayName || "User"}</div><div className="text-slate-500 dark:text-slate-500 text-xs truncate">{user.email}</div></div>}
            </button>
          </div>
        </div>
      </aside>
      ...
      <main className={`flex-1 h-full overflow-hidden flex flex-col transition-all duration-300 ease-in-out ${isSidebarCollapsed ? 'md:ml-20' : 'md:ml-64'}`}>
        <header className="shrink-0 flex justify-end items-center p-4 bg-slate-50/80 dark:bg-slate-950/80 backdrop-blur-sm z-30">
           <div><button onClick={toggleTheme} className="flex items-center gap-2 px-4 py-2 rounded-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm text-sm font-medium text-slate-600 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-all">{theme === 'dark' ? <Sun size={18} /> : <Moon size={18} />}<span>{theme === 'dark' ? 'Light' : 'Dark'}</span></button></div>
        </header>
        <div className="flex-1 overflow-y-scroll p-4 md:p-8 pt-0 pb-24 md:pb-8"><div className="max-w-7xl mx-auto w-full">{renderContent()}</div></div>
      </main>
      ...
      <Modal isOpen={isSettingsOpen} onClose={() => setIsSettingsOpen(false)} title="User Settings">
        <div className="space-y-6">
          <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800 flex items-center gap-4">
             {user.photoURL ? <img src={user.photoURL} alt="User" className="w-16 h-16 rounded-full border-2 border-white dark:border-slate-700 shadow-sm" /> : <div className="w-16 h-16 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center text-blue-600 dark:text-blue-400"><UserCircle size={32} /></div>}
             <div><h4 className="font-bold text-slate-900 dark:text-white">{user.displayName || "User"}</h4><p className="text-sm text-slate-500 dark:text-slate-400">{user.email}</p></div>
          </div>
          <div className="space-y-4">
             ...
             <div className="pt-4 mt-2 border-t border-slate-100 dark:border-slate-800">
               <button onClick={handleLogout} className="w-full p-3 rounded-xl border border-slate-200 text-slate-600 text-sm font-bold hover:bg-slate-50 transition-colors dark:border-slate-800 dark:text-slate-300 dark:hover:bg-slate-800 flex items-center justify-center gap-2"><LogOut size={16} /> Sign Out</button>
             </div>
          </div>
