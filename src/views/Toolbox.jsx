...
const Toolbox = ({ inventory, setInventory, currencySymbol }) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <Modal isOpen={deleteModal.isOpen} onClose={() => setDeleteModal({ ...deleteModal, isOpen: false })} title="Remove Tool?">
        <div className="space-y-6">
          <div className="flex items-start gap-4">
             ...
             <div><p className="text-slate-600 dark:text-slate-300">Are you sure you want to remove <strong>{deleteModal.toolName}</strong> from your inventory?</p></div>
          </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 justify-between items-center">
        <div className="flex p-1 bg-slate-200 dark:bg-slate-800 rounded-lg w-full md:w-auto">
          {['all', 'owned', 'needed'].map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-1.5 rounded-md text-sm font-bold capitalize transition-all flex-1 md:flex-none ${activeTab === tab ? 'bg-white dark:bg-slate-700 text-blue-600 dark:text-blue-400 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700'}`}>{tab}</button>
          ))}
        </div>
        <div className="flex gap-2 w-full md:w-auto">
          <div className="relative flex-1 md:w-64">
            <Search className="absolute left-3 top-2.5 text-slate-400" size={18} />
            <input type="text" placeholder="Filter tools..." className="w-full pl-10 pr-4 py-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
          </div>
          <Button variant="primary" icon={Plus} onClick={() => setIsAdding(!isAdding)}>Add Tool</Button>
        </div>
      </div>

      {/* Add Form with Overflow Visible to show dropdown */}
      {isAdding && (
        <Card className="p-5 bg-blue-50 dark:bg-blue-900/10 border-blue-100 dark:border-blue-900/30 animate-in slide-in-from-top-2 overflow-visible">
           <form onSubmit={handleAddTool} className="flex flex-col md:flex-row gap-4 items-start">
              <Input label="Tool Name" placeholder="e.g. Impact Driver" value={newTool.name} onChange={e => setNewTool({...newTool, name: e.target.value})} className="flex-1 w-full" autoFocus />
              <div className="w-full md:w-48">
                <Select label="Category" options={CATEGORIES} value={newTool.category} onChange={e => setNewTool({...newTool, category: e.target.value})} />
              </div>
              <Input label="Price" type="number" placeholder="0.00" value={newTool.price} onChange={e => setNewTool({...newTool, price: e.target.value})} className="w-full md:w-32" icon={<span className="text-xs font-bold text-slate-400">{currencySymbol}</span>} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {filteredTools.map((tool) => (
          <div key={tool.id} className={`group p-4 rounded-xl border transition-all duration-300 flex justify-between items-center relative overflow-hidden ${tool.owned ? 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 shadow-sm' : 'bg-slate-50 dark:bg-slate-800/30 border-dashed border-slate-300 dark:border-slate-700'}`}>
             <div className={`absolute left-0 top-0 bottom-0 w-1 ${tool.owned ? 'bg-emerald-500' : 'bg-amber-400'}`}></div>
             <div className="flex items-center gap-4 pl-2">
                <button onClick={() => toggleOwnership(tool.id)} className={`transition-colors p-1 rounded-full ${tool.owned ? 'text-emerald-500 hover:bg-emerald-50' : 'text-slate-300 hover:text-blue-500 hover:bg-blue-50'}`} title={tool.owned ? "Mark as Needed" : "Mark as Owned"}>
                ...
                <div>
                   ...
                   <div className="flex items-center gap-3 mt-1">
                      ...
                      <div className="flex items-center group/price cursor-text">
                        ...
                        <input type="number" className={`w-16 bg-transparent border-b border-transparent hover:border-slate-300 focus:border-blue-500 text-xs font-numbers focus:outline-none no-spin ${tool.owned ? 'text-emerald-600' : 'text-slate-500'}`} value={tool.price} onChange={(e) => handlePriceChange(tool.id, e.target.value)} />
                      </div>
                   </div>
                </div>
             </div>
             <button onClick={() => confirmDelete(tool)} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800"><Trash2 size={18} /></button>
          </div>
        ))}
      </div>
